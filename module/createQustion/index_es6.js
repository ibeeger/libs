 ...
 * @Last Modified time: 2017-02-24 17:49:22
 */
...
class Qustion {
	...
	static createQuestions(area, numarea, count) {
		var _cout = 1;
		let questions = [];
		while (_cout < count) {
			_cout++;
			...
			let json = this.getRandom(mthIndex, numarea);
			let first = json["first"] //参数1
			let second = json["second"] //参数2
			let answer = json["answer"];
			let arr = this.getChoose(answer);

			// console.log(first + name + second + "等于" + answer + " [" + arr + "]");

			questions.push({
				title:first + name + second + "等于多少呢？",
				answer:answer,
				options:arr 
			})
		}
...
Qustion.createQuestions(4, NumberArea, 15);